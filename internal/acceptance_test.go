...
func assertGradingResult(t testing.TB, got, want models.GradingResult) {
	...
	if got.IsSuccess != want.IsSuccess {
		t.Errorf("expected IsSuccess to be %t, instead got %t", want.IsSuccess, got.IsSuccess)
	}

	if len(got.TestcaseGradingResult) != len(want.TestcaseGradingResult) {
		t.Fatalf("expected got and want results to be the same number, instead got has %d and want has %d", len(got.TestcaseGradingResult), len(want.TestcaseGradingResult))
	}

	for i, _ := range got.TestcaseGradingResult {
		if got.TestcaseGradingResult[i].Verdict != want.TestcaseGradingResult[i].Verdict {
			t.Errorf("expected %q TC to have %s verdict, instead got %s", want.TestcaseGradingResult[i].InputFilename+"-"+want.TestcaseGradingResult[i].OutputFilename, want.TestcaseGradingResult[i].Verdict, got.TestcaseGradingResult[i].Verdict)
		}
